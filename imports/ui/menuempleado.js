...
export default class MenuEmployee extends React.Component {
  ...
  closeAgregar() {
    //cerrar modal que muestra info del platillo
    var modal = document.getElementById('simpleModalEmp');
    modal.style.display = 'none';
  }
...
class Tabla extends React.Component {
  ...
  componentDidMount() {
    //cargar arreglo de orders desde meteor subscriptions
  ...
  checkStatus(Order) {
    //revisa el estado de cada orden y regresa un span con el estado
    let jsx = '';
    if (Order.status == 'InProgress') {
      ...
      jsx = (
        <span>
          {'PENDIENTE '}
          <i className="fas fa-exclamation-triangle fa-2x red" />
        </span>
      );
  ...
  render() {
    //render de clase Tabla

    return <div>{this.loadTable()}</div>;
  }
  ...
  loadHistory() {
    ...
    return this.state.orders.map((order) => {
      ...
      function getUser(usuario) {
        return usuario._id === order.userId;
      }
      const user = this.state.users.find(getUser);
      console.log(user);
            ...
            <td>
              <button
                ...
                onClick={function() {
                  if (
                    window.confirm(
                      'Esta segur@ de esconder esta orden del sistema?'
                    )
                  ) {
                    //Meteor.call('orders.delete', order._id);
                    Meteor.call('orders.setHidden', order._id);
                    toastr.success(
                      'La orden ha sido escondida exitosamente del sistema!'
                    );
                  }
                }}
              >
  ...
  loadMenu() {
    //carga las ordenes pendientes/terminadas
    return this.state.orders.map((order) => {
      const user = Meteor.users.findOne({ _id: order.userId });
      if (order.status == '') {
              ...
              <button
                ...
                onClick={function() {
                  if (
                    ...
                    window.confirm(
                      '¿Esta seguro de cambiar esta orden a terminada?'
                    )
                  ) {
                    let d = new Date();
                    let stringFecha =
  ...
  loadTable() {
    //carga la tabla dependiendo del tipo (pendientes/ingresadas o terminadas)
